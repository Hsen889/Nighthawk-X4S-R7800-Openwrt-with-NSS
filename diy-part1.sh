...
rm -rf bin
# 删除自带cpufreq配置
rm -f target/linux/ipq806x/base-files/etc/init.d/bootcount
...
# sed -i 's/IMG_PREFIX:=$(VERSION_DIST_SANITIZED)/IMG_PREFIX:=Hsen889-$(shell date +%Y%m%d)-$(VERSION_DIST_SANITIZED)/g' include/image.mk
